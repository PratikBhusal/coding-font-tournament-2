...
const clampWidth = (width: number) => Math.min(Math.max(width, 192), window.innerWidth || width);
...
const defaultOpen = (aside: HTMLElement) =>
  aside.dataset.defaultOpen === '1' || window.matchMedia('(min-width: 1024px)').matches;
...
export function initSidebar() {
  ...
  document.addEventListener('pointerdown', (event) => {
    const handle = (event.target as HTMLElement | null)?.closest('#app-sidebar-resize');
    const current = getAside();
    if (!handle || !current) return;
    event.preventDefault();
    writeJSON('menuOpen', true);
    dragStartX = event.clientX;
    dragStartWidth = current.getBoundingClientRect().width || readJSON('sidebarWidth', defaultSidebarWidth());
